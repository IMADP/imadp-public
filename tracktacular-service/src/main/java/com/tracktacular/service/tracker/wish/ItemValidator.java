 ...
 *       {objectName}.url.invalidMaxLength
...
public class ItemValidator extends AbstractValidator<Item> {
	...
	private static final int MAX_URL_LENGTH = 25000;
	...
	protected void validate(String objectName, Item item, List<ValidationFailure> failures) {
		...
		String urlName = join(objectName, Item.URL);
		...
		addValidationResult(new MaxLengthValidator(urlName, item.getUrl(), MAX_URL_LENGTH));
