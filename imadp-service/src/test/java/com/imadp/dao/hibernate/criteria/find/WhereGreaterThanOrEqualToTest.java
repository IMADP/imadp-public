 ...
 */
@Transactional
public class WhereGreaterThanOrEqualToTest extends IMADPServiceTestCase {
	...
	public void persistableNumberEqualToTestedValue() throws Exception {		
		findCriteria = samplePersistableDao.findCriteriaBuilder(Results.ALL)
			.whereGreaterThanOrEqualTo(SamplePersistable.LONG_NUMBER, 5L).build();
	...
	public void persistableNumberLessThanTestedValue() throws Exception {		
		findCriteria = samplePersistableDao.findCriteriaBuilder(Results.ALL)
			.whereGreaterThanOrEqualTo(SamplePersistable.LONG_NUMBER, 10L).build();
		
		assertEquals(0, samplePersistableDao.findBy(findCriteria).size());
	}
	
	@Test
	public void persistableNumberGreaterThanOrEqualTestedValue() throws Exception {		
		findCriteria = samplePersistableDao.findCriteriaBuilder(Results.ALL)
			.whereGreaterThanOrEqualTo(SamplePersistable.LONG_NUMBER, 3L).build();
