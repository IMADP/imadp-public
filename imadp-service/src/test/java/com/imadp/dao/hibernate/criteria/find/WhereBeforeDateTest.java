 ...
 */
@Transactional
public class WhereBeforeDateTest extends IMADPServiceTestCase {
	...
	public void persistableDateBeforeTestedValue() throws Exception {
		...
		findCriteria = samplePersistableDao.findCriteriaBuilder(Results.ALL)
			.whereBeforeDate(SamplePersistable.START_DATE, date).build();
	...
	public void persistableDateOnTestedValue() throws Exception {		
		findCriteria = samplePersistableDao.findCriteriaBuilder(Results.ALL)
			.whereBeforeDate(SamplePersistable.START_DATE, startDate).build();
