 ...
 */
@Transactional
public class WhereBeforeOrOnDateTest extends IMADPServiceTestCase {
	...
	public void persistableDateBeforeTestedValue() throws Exception {
		...
		findCriteria = samplePersistableDao.findCriteriaBuilder(Results.ALL)
			.whereBeforeOrOnDate(SamplePersistable.START_DATE, date).build();
	...
	public void persistableDateOnTestedValue() throws Exception {		
		findCriteria = samplePersistableDao.findCriteriaBuilder(Results.ALL)
			.whereBeforeOrOnDate(SamplePersistable.START_DATE, startDate).build();
