 ...
 */
@Transactional
public class WhereAfterDateTest extends IMADPServiceTestCase {
	...
	public void persistableDateAfterTestedValue() throws Exception {		
		...
		findCriteria = samplePersistableDao.findCriteriaBuilder(Results.ALL)
			.whereAfterDate(SamplePersistable.START_DATE, date).build();
	...
	public void persistableDateOnTestedValue() throws Exception {		
		findCriteria = samplePersistableDao.findCriteriaBuilder(Results.ALL)
			.whereAfterDate(SamplePersistable.START_DATE, startDate).build();
